        ...
        try {
            ...
            if (userService.getUserPermission(user) == -1) {
                if (user.getId() != solution.getUser().getId()) {
                    ...
                    solution.setSource("This Source Code Is Not Shared!");
                    solution.setInfo(null);
                } else if (solution.getShare() && user.getUserProfile() != null && 
                          user.getUserProfile().getScore() < config.getLeastScoreToSeeOthersCode()) {
            ...
            log.error("Error showing source code for solution: {}", id, e);
            throw new NotFoundException("Error loading solution");
        }
    }
