...
public class SolutionService {
    ...
    public Solution updateSolutionResultInfo(Solution solution) {
        String info = solution.getInfo();
        if (info == null) {
            info = "";
            solution.setInfo(info);
        }
        try {
            solutionRepository.updateResultInfo(solution.getId(), solution.getResult(), info);
        } catch (DataIntegrityViolationException e) {
            // 兼容旧库结构（info 可能仍是 VARCHAR(255)）；避免回调失败导致一直 Pending
            final String suffix = " ...(truncated)";
            final int maxLen = 254 - suffix.length(); // 尽量保证 <= 255
            String truncated = info;
            if (info.length() > maxLen) {
                truncated = info.substring(0, maxLen) + suffix;
            }
            log.warn("Solution {} info too long, truncating before saving (len={} -> {})",
                    solution.getId(), info.length(), truncated.length(), e);
            solution.setInfo(truncated);
            solutionRepository.updateResultInfo(solution.getId(), solution.getResult(), truncated);
        }
        return solution;
    }
    ...
    public Page<Solution> getSolutionsInContest(int page, int size, Contest contest) {
        return solutionRepository.findAllByContestOrderByIdDesc(PageRequest.of(page, size, Sort.by(Sort.Direction.DESC, "id")), contest);
    }
