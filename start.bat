...
if %errorlevel% neq 0 (
    echo 正在初始化数据库...
    type "%SQL_FILE%" | docker exec -i oj-mysql mysql -u root -p1234
    if !errorlevel! equ 0 (
        echo [成功] 数据库初始化完成
    ) else (
...
java -Dfile.encoding=UTF-8 -Dstdout.encoding=UTF-8 -Dstderr.encoding=UTF-8 -jar "%APP_JAR%" --spring.datasource.url="jdbc:mysql://localhost:13306/oj?serverTimezone=GMT%%2B8&useUnicode=yes&characterEncoding=UTF-8" --spring.redis.host=localhost --spring.redis.port=16379
